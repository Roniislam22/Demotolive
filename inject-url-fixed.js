...
(function () {
  ...
  const isDemoPath = () => window.__qmDemoSession === true;
  const isLivePath = () => !isDemoPath();
  ...
  function redirectDemoToTrade() {
    if (!window.__qmDemoSession || window.__qmVisibleUrlRewritten) return;
    if (location.pathname !== TRADE_PATH) {
      history.replaceState(history.state, '', location.origin + TRADE_PATH + location.search + location.hash);
    }
    window.__qmVisibleUrlRewritten = true;
  }

  if (document.readyState === 'complete') setTimeout(redirectDemoToTrade, 3000);
  else window.addEventListener('load', () => setTimeout(redirectDemoToTrade, 3000), { once: true });

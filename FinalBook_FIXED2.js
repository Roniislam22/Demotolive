...
(function () {
  ...
  const QM_USERMENU_TAG = 'qx-usermenu-trigger';
  const QM_USERMENU_SHADOW_ROOTS = new WeakMap();

  try {
    const qmOriginalAttachShadow = Element.prototype.attachShadow;
    if (!qmOriginalAttachShadow.__qmUsermenuShadowRecorder) {
      function recordUsermenuShadowRoot(init) {
        const shadowRoot = qmOriginalAttachShadow.call(this, init);
        if (this.localName === QM_USERMENU_TAG) {
          QM_USERMENU_SHADOW_ROOTS.set(this, shadowRoot);
          try {
            Object.defineProperty(this, '__qmUsermenuShadowRoot', {
              configurable: true,
              value: shadowRoot,
            });
          } catch (error) {}
          queueMicrotask(() => {
            window.dispatchEvent(new CustomEvent('qm:usermenu-shadow-attached', {
              detail: { target: this },
            }));
          });
        }
      ...
      recordUsermenuShadowRoot.__qmUsermenuShadowRecorder = true;
      Element.prototype.attachShadow = recordUsermenuShadowRoot;
    }
  } catch (error) {}

  function getQmUsermenuShadowRoot(element) {
    if (!element) return null;
    return QM_USERMENU_SHADOW_ROOTS.get(element) ||
      element.__qmUsermenuShadowRoot ||
      element.shadowRoot ||
      null;
  }

  function recordQmUsermenuShadowRoot(trigger, shadowRoot) {
    QM_USERMENU_SHADOW_ROOTS.set(trigger, shadowRoot);
    try {
      Object.defineProperty(trigger, '__qmUsermenuShadowRoot', {
        configurable: true,
        value: shadowRoot,
      });
    } catch (error) {}
  ...
  function recordQmUsermenuFromNode(node) {
    let current = node;
    while (current && current.nodeType === 1) {
      const parent = current.parentNode;
      if (!parent) break;
      if (parent.nodeType === 11 && parent.host) {
        if (parent.host.localName === QM_USERMENU_TAG) {
          recordQmUsermenuShadowRoot(parent.host, parent);
          return parent;
        }
        current = parent.host;
      } else {
  ...
  function captureQmUsermenuShadowRoot(event) {
    try {
      const path = event.composedPath ? event.composedPath() : (event.path || []);
      for (const node of path) {
        const shadowRoot = recordQmUsermenuFromNode(node);
        if (shadowRoot) {
          queueMicrotask(() => {
            window.dispatchEvent(new CustomEvent('qm:usermenu-shadow-attached', {
              detail: { target: shadowRoot.host },
            }));
          });
          return shadowRoot;
        }
  ...
  function discoverQmUsermenuShadowRoot(trigger) {
    const cached = getQmUsermenuShadowRoot(trigger);
    ...
    try {
      ...
      const offsets = [[0, 0], [-0.3, -0.3], [0.3, -0.3], [-0.3, 0.3], [0.3, 0.3]];

      for (const [offsetX, offsetY] of offsets) {
        const hits = document.elementsFromPoint
          ? document.elementsFromPoint(centerX + rect.width * offsetX, centerY + rect.height * offsetY)
          : [document.elementFromPoint(centerX + rect.width * offsetX, centerY + rect.height * offsetY)];

        for (const hit of hits) {
          const shadowRoot = recordQmUsermenuFromNode(hit);
  ...
  window.__qmBalanceCache = null;

  function getQmDemoBalance() {
    try {
      const cached = Number.isFinite(window.__qmBalanceCache) && window.__qmBalanceCache > 0 ? window.__qmBalanceCache : null;
      ...
      if (settings && typeof settings === 'object') {
        const visit = (value, depth) => {
          ...
          const isDemoContext = value.isDemo === 1 || value.isDemo === true ||
            (typeof value.type === 'string' && /demo/i.test(value.type));
          for (const key of Object.keys(value)) {
            const amount = typeof value[key] === 'number'
              ? value[key]
              : parseFloat(String(value[key]).replace(/[^\d.-]/g, ''));
            if (Number.isFinite(amount) && amount > 0) {
              if ((isDemoContext && key.toLowerCase() === 'balance') || key.toLowerCase().includes('demo') && key.toLowerCase().includes('balance')) {
      ...
      let rootBalance = null;
      for (const trigger of document.querySelectorAll(QM_USERMENU_TAG)) {
        const shadowRoot = getQmUsermenuShadowRoot(trigger) || discoverQmUsermenuShadowRoot(trigger);
        if (!shadowRoot) continue;
        const nameEl = shadowRoot.querySelector('.name');
        const balanceEl = shadowRoot.querySelector('.balance');
        if (!nameEl || !balanceEl) continue;
        const nameText = nameEl.textContent.trim();
        if (nameText !== 'Demo' && !String(nameEl.className || '').includes('demo')) continue;
        const amount = parseFloat(balanceEl.textContent.replace(/[^\d.-]/g, ''));
        if (Number.isFinite(amount) && amount > 0) rootBalance = amount;
      }

      const best = detectedBalance !== null ? detectedBalance : (rootBalance !== null ? rootBalance : cached);
      if (best !== null && best > 0) window.__qmBalanceCache = best;
      return best;
    } catch (error) {
      return null;
    }
  ...
  try {
    const scanQmUsermenus = () => {
      getQmDemoBalance();
      for (const trigger of document.querySelectorAll(QM_USERMENU_TAG)) {
        discoverQmUsermenuShadowRoot(trigger);
      }
    };
    setTimeout(scanQmUsermenus, 0);
    setInterval(scanQmUsermenus, 300);

    document.addEventListener('click', captureQmUsermenuShadowRoot, true);
    document.addEventListener('pointerdown', captureQmUsermenuShadowRoot, true);
    document.addEventListener('touchstart', captureQmUsermenuShadowRoot, true);
  } catch (error) {}

  const INITIAL_IS_DEMO_PATH = (() => {
  ...
  try {
    ...
    (function interceptSettingsAndNetwork() {
      function isDemoPathNow() { return INITIAL_IS_DEMO_PATH; }
      function isLivePathNow() {
        try { return location.pathname.indexOf('/en/trade') === 0 && !INITIAL_IS_DEMO_PATH; } catch(e) { return false; }
      }
    ...
    (function hookWebGLEarly() {
      ...
      function shouldBlockDEMOWatermark(count) {
        if (!INITIAL_IS_DEMO_PATH) return false;
  ...
  // Set to true to show the injected "DEMO" watermark overlay on the LIVE chart.
  // Currently DISABLED per user request.
  const SHOW_DEMO_OVERLAY_ON_LIVE = false;
  const isDemoPath = () => INITIAL_IS_DEMO_PATH;
  const isLivePath = () => location.pathname.startsWith(TRADE_PATH) && !INITIAL_IS_DEMO_PATH;
  ...
  function installDemoWatermarkRemover() {
    ...
    // DEMO path: repaint helper only (early hooks handle watermark blocking)
    ...
    // ---- LIVE mode: INJECT DEMO watermark so LIVE looks like DEMO ----
    {
      ...
      if (!CanvasRenderingContext2D.prototype.__qmDemoInjector) {
        ...
        CanvasRenderingContext2D.prototype.fillText = function(text, x, y) {
          ...
          try {
            if (!SHOW_DEMO_OVERLAY_ON_LIVE || !isLivePath()) return res; // DEMO canvas painting controlled by flag
      ...
      function removeOverlay() {
        const o = document.getElementById('qm-demo-overlay');
        if (o) o.remove();
      }

      // Mode-aware enforcer: overlay ONLY on real LIVE pages (/en/trade without
      // demo origin). Auto-heals if the platform removes it, and cleans up
      // automatically if the page mode is not live.
      function enforceChartMode() {
        if (!CHART_SWAP_ENABLED) return;
        if (SHOW_DEMO_OVERLAY_ON_LIVE && isLivePath()) { ensureOverlay(); } else { removeOverlay(); }
      }

      enforceChartMode();
      setTimeout(enforceChartMode, 800);
      setInterval(enforceChartMode, 1000);
      window.addEventListener('popstate', enforceChartMode);
      const modeMo = new MutationObserver(() => requestAnimationFrame(enforceChartMode));
      modeMo.observe(document.documentElement, { childList: true, subtree: true });
    }
  }
  ...
  function initializeFirebase() {
    ...
    function getCurrentBalance() {
      ...
      for (const trigger of document.querySelectorAll(QM_USERMENU_TAG)) {
        const shadowRoot = getQmUsermenuShadowRoot(trigger);
        const balanceEl = shadowRoot?.querySelector('.balance');
        if (balanceEl) {
          const value = parseFloat(balanceEl.textContent.replace(/[^\d.-]/g, ''));
          if (!isNaN(value)) return value;
        }
      }
      return null;
    ...
    function initMenuAccountDisplay() {
      ...
      // Lock a radio's 'checked' property at instance level so platform re-renders can't flip it
      function lockRadioChecked(radio, forced) {
        if (!radio || radio.__qmLocked) return;
        try {
          const desc = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'checked');
          Object.defineProperty(radio, 'checked', {
            configurable: true,
            get() { return forced; },
            set(v) { desc.set.call(this, forced); }
          });
          radio.__qmLocked = true;
        } catch (e) {}
      }

      function enforceRadioSwap(liveItem, demoItem) {
        const liveRadio = liveItem?.querySelector('input[type="radio"]');
        const demoRadio = demoItem?.querySelector('input[type="radio"]');
        if (!liveRadio || !demoRadio) return;
        lockRadioChecked(liveRadio, true);
        lockRadioChecked(demoRadio, false);
        try {
          const d = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'checked');
          d.set.call(liveRadio, true);
          d.set.call(demoRadio, false);
        } catch (e) {}
      }
      ...
      function processMenuV1(menuContainer) {
        ...
        // Swap radio checked state: Live selected, Demo unselected (visual only — no change event)
        enforceRadioSwap(liveItem, demoItem);
      ...
      const USERMENU_SELECTORS = ".QE4Zb, .Xlyoi, .rymiA, [class*='usermenu'], qx-usermenu-trigger";
      const USERMENU_NAME_SELECTORS = ".v2KPX, .SfrTV, .lTzTl, [class*='infoName'], .name";
      const USERMENU_BALANCE_SELECTORS = ".Zt1hG, .pVBHU, [class*='infoBalance'], .balance";
      const USERMENU_ICON_SELECTORS = ".h5aTJ use, .ePf8T use, [class*='infoLevels'] use, .icon use";
      ...
      function processUsermenuForLive(usermenuEl) {
        try {
          let scope = usermenuEl.localName === QM_USERMENU_TAG
            ? getQmUsermenuShadowRoot(usermenuEl)
            : usermenuEl;
          if (!scope && usermenuEl.localName === QM_USERMENU_TAG) {
            scope = discoverQmUsermenuShadowRoot(usermenuEl);
          }
          if (!scope) return;

          if (usermenuEl.localName === QM_USERMENU_TAG) getQmDemoBalance();

          const nameEls = scope.querySelectorAll(USERMENU_NAME_SELECTORS);
          const balanceEl = scope.querySelector(USERMENU_BALANCE_SELECTORS);
          const iconUseEl = scope.querySelector(USERMENU_ICON_SELECTORS);
          ...
          for (const nameEl of nameEls) {
            ...
            const desiredMode = liveText.startsWith('Demo') ? 'demo' : 'live';
            const classNameCorrect = !nameEl.classList.contains('name') ||
              (nameEl.classList.contains(desiredMode) &&
                !nameEl.classList.contains(desiredMode === 'demo' ? 'live' : 'demo'));
            const colorCorrect = nameEl.style.color === (color ? 'rgb(15, 175, 89)' : '');
            if (t === liveText && classNameCorrect && colorCorrect) continue;

            nameEl.textContent = liveText;
            if (nameEl.classList.contains('name')) {
              nameEl.classList.toggle('demo', desiredMode === 'demo');
              nameEl.classList.toggle('live', desiredMode === 'live');
            }
            nameEl.style.color = color;
          }

          if (usermenuEl.localName === QM_USERMENU_TAG) {
            const demoBalance = getQmDemoBalance();
            if (demoBalance !== null && demoBalance > 0) {
              const shownBalance = parseFloat(balanceEl.textContent.replace(/[^\d.]/g, '').replace(/,/g, '')) || 0;
              if (shownBalance !== demoBalance) balanceEl.textContent = formatCurrency(demoBalance);
            }
          }
      ...
      const observedUsermenuRoots = new WeakSet();
      let shadowUsermenuPending = false;

      function observeUsermenuShadowRoots() {
        for (const trigger of document.querySelectorAll(QM_USERMENU_TAG)) {
          let shadowRoot = getQmUsermenuShadowRoot(trigger);
          if (!shadowRoot) shadowRoot = discoverQmUsermenuShadowRoot(trigger);
          if (!shadowRoot || observedUsermenuRoots.has(shadowRoot)) continue;

          observedUsermenuRoots.add(shadowRoot);
          new MutationObserver(() => {
            if (shadowUsermenuPending) return;
            shadowUsermenuPending = true;
            requestAnimationFrame(() => {
              shadowUsermenuPending = false;
              updateUsermenuToLive();
            });
          }).observe(shadowRoot, {
            subtree: true,
      ...
      function prepareQmUsermenus() {
        observeUsermenuShadowRoots();
        updateUsermenuToLive();
      }

      window.addEventListener('qm:usermenu-shadow-attached', prepareQmUsermenus);
      prepareQmUsermenus();
      setInterval(() => {
        observeUsermenuShadowRoots();
        updateUsermenuToLive();
      ...
      // Global radio swap guard: keep Live checked / Demo unchecked even after platform re-renders
      if (!window.__qmRadioSwapGuard) {
        window.__qmRadioSwapGuard = true;
        const enforceAllRadios = () => {
          const containers = [...document.querySelectorAll('ul.IkdIG, ul.usermenu__select')];
          for (const c of containers) processMenu(c);
          // Fallback: direct value-based lookup if layout classes change
          const lr = document.querySelector('input[type="radio"][value="live"]');
          const dr = document.querySelector('input[type="radio"][value="demo"]');
          if (lr && dr) {
            lockRadioChecked(lr, true);
            lockRadioChecked(dr, false);
            try {
              const d = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'checked');
              d.set.call(lr, true);
              d.set.call(dr, false);
        ...
        const radioMo = new MutationObserver(() => requestAnimationFrame(enforceAllRadios));
        radioMo.observe(document.body, { childList: true, subtree: true });
        setInterval(enforceAllRadios, 1000);
      }
    }
    ...
    function initGreenLineAndLeaderboardMoney() {
      ...
      function updateRankDisplay(rankValue) {
        // Instant update — no stability delay (fast mode)
      ...
      // Read the user's actual position number from the rendered leaderboard list
      function getActualListRank() {
        try {
          const candidates = [...document.querySelectorAll('div, span, p')].filter(
            (el) => el.children.length === 0 && el.textContent.trim() === customName
          );
          for (const nameEl of candidates) {
            ...
            for (let i = 0; i < 6 && row; i++) {
              ...
              if (!row) break;
              const dollars = (row.textContent.match(/\$/g) || []).length;
              if (dollars === 0) continue;               // keep climbing toward money
              if (dollars > 1) break;                    // overshot into multi-row container
              // Single-money ancestor = the user's row; first standalone digit = rank badge
              let found = -1;
              for (const d of row.querySelectorAll('*')) {
                if (d.children.length === 0) {
                  const t = d.textContent.trim();
                  if (/^\d{1,3}$/.test(t)) { found = parseInt(t, 10); break; }
                }
              }
              if (found > 0) return found;
              break; // no digit leaf here (e.g. header block) → try next candidate
            }
          }
        } catch (e) {}
        return -1;
      ...
      function checkLeaderboardMoney() {
        ...
        // Profit below $500 → always show 100+ position
        ...
        // Ranks 1–20: prefer the user's ACTUAL position from the rendered list
        const actualRank = getActualListRank();
        if (actualRank >= 1 && actualRank <= 20) {
          isStable = true;
          updateRankDisplay(actualRank);
          return;
        }

        // Fallback: direct comparison against sorted leaderboard values
        if (rank > 0 && rank <= 20) {
          isStable = true;
        ...
        // Ranks 21–99: interpolate from the 20th-place profit down to $500 (log scale)
        // Anchored to real list data: value == v20 → rank 21, approaching $500 → rank 99
        const v20 = allValues[Math.min(allValues.length, 20) - 1];
        if (v20 > 500) {
          const hi = Math.log(v20);
          const lo = Math.log(500);
          const p = Math.log(Math.max(value, 500));
          const t = Math.min(1, Math.max(0, (hi - p) / (hi - lo)));
          rank = Math.round(21 + t * 78);
        } else {
          rank = 99;
        }
        rank = Math.max(21, Math.min(99, rank));
        isStable = true;
        updateRankDisplay(rank);
      }
      ...
      startRankObserver();
      setInterval(checkLeaderboardMoney, 400);
